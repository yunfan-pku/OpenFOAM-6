...
class tabulation:public simple_tabulation,public RTree<coord_int, double,3,double>
{
	public:
		...
		int Search_(const double a[]);
		void getBox(double min[],double max[],Point arr_point[]);
		friend bool SearchCallback(coord_int  id, void* arg);
		double interpolation_tab(double x,double y,double z,int index);
		static inline double mixed_product(const Point& a_pointA,const Point& a_pointB,const Point& a_pointC,const Point& a_pointD);
		static inline double abs(const double& a);
		static inline double cross_2d(const Point& a_pointA,const Point& a_pointB,const Point& a_pointC);
		static bool point_in_tab_quad_2d(const Point& a_point, const Quad_2d& a_quad_2d);
...
bool tabulation::point_in_tab_quad_2d(const Point& a_point, const Quad_2d& a_quad_2d,coord_int  id)
{
	...
	a3=A4*B3-A3*B4;
	double x,y,delta=a2*a2-4*a1*a3;
	if(delta<0)
		return false;
	if(abs(a1)<eps)
	{
...
bool SearchCallback(coord_int  id, void* arg)
{
	...
	{
		This->ret_coord=id;
		if(id.m[0]==0||id.m[1]==0||id.m[2]==0||id.m[0]==This->dnum[0]-1||id.m[1]==This->dnum[1]-1||id.m[2]==This->dnum[2]-1)
		return true;
		return false;
	}
